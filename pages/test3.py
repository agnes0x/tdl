...
with st.sidebar:
    ...
    with chat_container:
        current_chat = st.radio(
            ...
            # on_change=current_chat_callback  # Callbacks are not suitable here，cannot change windows
        )
    st.write("---")
...
with st.sidebar:
    ...
    st.write("\n")
    st.write("\n")
    st.text_input("set window name：", key="set_chat_name", placeholder="点击输入")
    st.selectbox(
    ...
    st.caption(
        """
    - double click to locate input
    - Ctrl + Enter submit questions
    """
    )
    st.markdown(
...
tap_input, tap_context, tap_model, tab_func = st.tabs(
    ["💬 chat", "🗒️ Default", "⚙️ Model", "🛠️ Settings"]
)
...
with tap_model:
    ...
    st.slider(
        ...
        args=("context_level",),
        help="Nr of convos historical，default excluded。",
    )
